...
\begin{document}
	...
	Um mit Nagios Core ein funktionsfähiges Monitoring erstellen zu können, sind immer mindestens zwei Komponenten nötig, zum einen Nagios Core selbst, zum anderen die Nagios Plugins, die die eigentliche Überwachung erledigen. Möchte man \acrlong{bspw} eine grafische Auswertung bestimmter Daten, so muss man auf Addons wie \textit{Nagios2RRD(N2RRD)}\footcite{n2rrdprojecthome} zurückgreifen. Dies gilt für viele Zusatzaufgaben, sei es grafische Aufbereitung oder das bequemere Monitoring von Verfügbarkeitszeiten, es empfiehlt sich auf jeden Fall einen Blick auf die verfügbaren Module zu werfen, die Auswahl ist reichhaltig.	
	...
	Wie sieht ein typischer Rückgabewert von einem Nagios Plugin nun aus? 
	
	%TODO wie wird geprüft, unsicher deshalb NRPE für Remote
	...
	\begin{figure}[!h]
		...
		\label{fig:checkmk}
	\end{figure}
	Die größte Besonderheit, ist aber das Check\_MK, wie in Abbildung \ref{fig:checkmkaufbau}
	\begin{figure}[!h]
		\centering
