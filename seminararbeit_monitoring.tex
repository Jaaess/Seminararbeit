...
\usepackage[a4paper,left=4.00cm, right=2.00cm, head=4.00cm,bottom=1.5cm,foot=0.5cm]{geometry}
...
\begin{document}
	...
	Woher weiß das Plugin welchen Status es zurückgeben muss? Dies wird über Schwellwerte beim Aufruf (von Nagios oder NRPE, je nachdem wer das Plugin aufruft) bestimmt, es gibt einen Schwellwert für Critical (\textit{-c}) und einen für Warning (\textit{-w}). Einen Check Aufruf mit Warning und Critical Schwellwerten sieht man in Listing \ref{lst:checkdisk}, wobei die Werte hier absichtlich so gewählt wurden, dass ein anderes Ergebnis als DISK OK zurückkommt, realistische Einstellungen wären, 20\% für Warning und 10\% für Critical.
...
%TODO Notification?
	...
	Als Fazit aus der (kurzen) Betrachtung der Funktionsweise von Nagios lässt sich schließen, das Nagios ein mächtiges Tool zur Netzwerküberwachung ist, aber sein der größter Vorteil, auch der größte Nachteil ist, durch die modulare Erweiterbarkeit und die extreme Vielfalt braucht es auch erstmal einiger Einarbeitung um ein wirklich funktionierendes Monitoring konfigurieren zu können. Das alle benötigten Daten von den Systemen bekommt und diese aufbereitet.
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=1\textwidth]{pics/hostServices.eps}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=1\textwidth]{pics/monitoring.eps}
